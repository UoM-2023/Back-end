...
async function addUtilityCharge(req, res) {
  try {
    ...
    electricityCost += await calculateUtilityCost(
      "Electricity",
      electricityUsage
    );

    const [balance] = await connection.query(
      "SELECT utility_balance FROM balance WHERE unit_id = ?",
      [unit_id]
    );
...
async function calculateUtilityCost(utilityType, numOfUnits) {
  ...
  const query =
    "SELECT * FROM utility_prices up JOIN utilityDetails ud ON ud.utility_id = up.utility_id WHERE ud.utility_name = ?";
  ...
  if (pricing.length > 0) {
    ...
    res
      .status(404)
      .json({ message: `Utility pricing details not found for ${type}.` });
    return;
  }
}
