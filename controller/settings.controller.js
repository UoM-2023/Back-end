...
async function updatePassword(req, res) {
  try {
    ...
    const [rows] = await connection.query(checkPasswordQuery, [UserID]);

    if (rows.length === 0 || rows.password !== oldPassword) {
    ...
    const updatePasswordQuery =
      "UPDATE User_Credentials SET userPassword = ? WHERE UserID = ?";
    await connection.query(updatePasswordQuery, [userPassword, UserID]);
